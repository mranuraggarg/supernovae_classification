...
def save_data(output, format="json"):
    """
    Saves the output of the load_data function in a chosen format.
    - format: "json" (default), "npz", or "pkl"
    """

    (X_train, X_train_reverse, Y_train, ids_train), (X_test, X_test_reverse, Y_test, ids_test), metadata = output

    if format == "json":
        data_dict = {
            "X_train": X_train.tolist(),
            "X_train_reverse": X_train_reverse.tolist(),
            "Y_train": Y_train.tolist(),
            "ids_train": ids_train.tolist(),
            "X_test": X_test.tolist(),
            "X_test_reverse": X_test_reverse.tolist(),
            "Y_test": Y_test.tolist(),
            "ids_test": ids_test.tolist(),
            "metadata": metadata
        }
        with open("supernovae_dataset.json", "w") as f:
            json.dump(data_dict, f, indent=4)
        print("Data saved in JSON format: saved_data.json")

    elif format == "npz":
        np.savez("supernovae_dataset.npz",
                 X_train=X_train, X_train_reverse=X_train_reverse, Y_train=Y_train, ids_train=ids_train,
                 X_test=X_test, X_test_reverse=X_test_reverse, Y_test=Y_test, ids_test=ids_test,
                 metadata=np.array(metadata, dtype=object))
        print("Data saved in NPZ format: saved_data.npz")

    elif format == "pkl":
        with open("supernovae_dataset.pkl", "wb") as f:
            pickle.dump(output, f)
        print("Data saved in Pickle format: saved_data.pkl")
    
    else:
        raise ValueError("Unsupported format. Choose 'json', 'npz', or 'pkl'.")

def load_saved_data(format="npz"):
    """
    Loads the previously saved supernova classification dataset.
    
    - format: "json" (default), "npz", or "pkl"
    - Returns: X_train, Y_train, X_test, Y_test, X_train_reverse, X_test_reverse, ids_train, ids_test, metadata
    """

    if format == "json":
        with open("supernovae_data.json", "r") as f:
            data = json.load(f)
        X_train = np.array(data["X_train"])
        X_train_reverse = np.array(data["X_train_reverse"])
        Y_train = np.array(data["Y_train"])
        ids_train = np.array(data["ids_train"])
        X_test = np.array(data["X_test"])
        X_test_reverse = np.array(data["X_test_reverse"])
        Y_test = np.array(data["Y_test"])
        ids_test = np.array(data["ids_test"])
        metadata = data["metadata"]
        print("Loaded data from JSON format.")

    elif format == "npz":
        data = np.load("supernovae_dataset.npz", allow_pickle=True)
        X_train = data["X_train"]
        X_train_reverse = data["X_train_reverse"]
        Y_train = data["Y_train"]
        ids_train = data["ids_train"]
        X_test = data["X_test"]
        X_test_reverse = data["X_test_reverse"]
        Y_test = data["Y_test"]
        ids_test = data["ids_test"]
        metadata = data["metadata"]
        print("Loaded data from NPZ format.")

    elif format == "pkl":
        with open("supernovae_classification.pkl", "rb") as f:
            output = pickle.load(f)
        (X_train, X_train_reverse, Y_train, ids_train), (X_test, X_test_reverse, Y_test, ids_test), metadata = output
        print("Loaded data from Pickle format.")

    else:
        raise ValueError("Unsupported format. Choose 'json', 'npz', or 'pkl'.")

    return X_train, Y_train, X_test, Y_test, X_train_reverse, X_test_reverse, ids_train, ids_test, metadata

def print_variable_shapes(X_train, Y_train, X_test, Y_test, X_train_reverse, X_test_reverse, ids_train, ids_test, metadata):
    """
    Prints the shape/length of all loaded dataset variables to verify correctness.
    """

    print(f"X_train shape: {X_train.shape}")
    print(f"X_train_reverse shape: {X_train_reverse.shape}")
    print(f"Y_train shape: {Y_train.shape}")
    print(f"ids_train length: {len(ids_train)}")

    print(f"X_test shape: {X_test.shape}")
    print(f"X_test_reverse shape: {X_test_reverse.shape}")
    print(f"Y_test shape: {Y_test.shape}")
    print(f"ids_test length: {len(ids_test)}")

    print(f"Metadata: {metadata}")  # Prints metadata (e.g., dimensions, num classes)

if __name__ == "__main__":
    # Load Data from the preprocessed files
    output = load_data()

    # Save data for later use
    save_data(output, format="npz")  # Change format to "json" or "pkl" if needed
